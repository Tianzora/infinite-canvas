...
import { refreshAllSourcesDetailed, refreshSource } from "@/services/api/prompts";
...
export function ConfigPromptSources() {
    ...
    const handleRefreshAll = async () => {
        ...
        try {
            const result = await refreshAllSourcesDetailed();
            if (!result.failed.length) updateSchedule("lastFetchedAt", new Date().toISOString());
            await invalidatePrompts();
            if (result.failed.length) message.warning(`已拉取 ${result.count} 条；失败来源：${result.failed.join("、")}`);
            else message.success(`全部来源已拉取，共 ${result.count} 条`);
        } catch (error) {
            message.error(error instanceof Error ? error.message : "拉取失败");
        } finally {
