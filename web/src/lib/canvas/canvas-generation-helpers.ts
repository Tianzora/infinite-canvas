...
import { retainImage, resolveImageUrl, type UploadedImage } from "@/services/image-storage";
import { resolveMediaUrl } from "@/services/file-storage";
import { imageMetadata } from "@/lib/canvas/canvas-node-factory";
import type { NodeGenerationInput } from "@/app/(user)/canvas/components/canvas-node-generation";
import type { CanvasImageAngleParams } from "@/app/(user)/canvas/components/canvas-node-angle-dialog";
import type { ReferenceImage } from "@/types/image";
import { CanvasNodeType, type CanvasAssistantSession, type CanvasConnection, type CanvasNodeData, type CanvasNodeMetadata, type CanvasGenerationMode } from "@/app/(user)/canvas/types";

export { isHiddenBatchChild, isHiddenBatchConnectionEndpoint } from "./canvas-node-geometry";
...
export function generationReferenceUrls(context: { referenceImages: ReferenceImage[]; referenceVideos: Array<{ storageKey?: string; url?: string }>; referenceAudios?: Array<{ storageKey?: string; url?: string }> }) {
    return [
        ...context.referenceImages.map((image) => image.storageKey || image.url || (!image.dataUrl.startsWith("data:") ? image.dataUrl : undefined)).filter((url): url is string => Boolean(url)),
    ...
    return Promise.all(
        nodes.map(async (node) => {
            ...
            if (node.metadata?.storageKey) {
                ...
                if (image.missing || !image.url) return { ...node, metadata: { ...node.metadata, content: "", status: "error" as const, errorDetails: "图片文件缺失，请重新上传或重新导入包含图片文件的画布包。" } };
            ...
            if (image.missing || !image.storageKey) return { ...node, metadata: { ...node.metadata, content: "", status: "error" as const, errorDetails: "图片文件缺失，请重新上传图片。" } };
...
export async function hydrateAssistantImages(sessions: CanvasAssistantSession[]) {
    ...
    return Promise.all(
        sessions.map(async (session) => ({
            ...session,
            messages: await Promise.all(session.messages.map(async (message) => ({ ...message, references: await Promise.all((message.references || []).map(hydrateItem)) }))),
        })),
    );
}
